...
export default async function TourDetail({params}:{params:Promise<{slug:string}>}){
  ...
  return <main><Header transparent/>
    ...
    <section className="tour-detail shell"><article className="tour-copy"><p className="eyebrow"><span/> La experiencia</p><h2>Una forma distinta de conocer {tour.location}.</h2><p className="lead">{tour.short}</p><p>Prepárate para descubrir paisajes extraordinarios acompañado por un guía local. Mantenemos grupos pequeños para cuidar cada detalle, movernos con respeto y disfrutar el recorrido sin prisa.</p>
      ...
      <div className="included-grid"><section className="included-card"><h3><span>✓</span>Qué incluye</h3><ul>{included.map(item=><li key={item}>✓ <span>{item}</span></li>)}</ul></section><section className="excluded-card"><h3><span>×</span>Qué no incluye</h3><ul>{excluded.map(item=><li key={item}>× <span>{item}</span></li>)}</ul></section></div>
      ...
      <section className="recommendation-card"><h3>△ Recomendaciones del recorrido</h3><ul>{recommendations.map(item=><li key={item}>{item}</li>)}</ul></section>
      ...
      <div className="gallery"><img src={tour.image} alt={`${tour.name}, vista principal`}/><img src="https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=900&q=85" alt="Paisaje de aventura en Panamá"/><img src="https://images.unsplash.com/photo-1528127269322-539801943592?auto=format&fit=crop&w=900&q=85" alt="Naturaleza tropical"/></div>
      <section className="review-form"><h2>¿Ya viviste esta aventura?</h2><p>Comparte tu experiencia. Todas las reseñas pasan por moderación antes de publicarse.</p><ReviewForm tourSlug={tour.slug}/></section>
