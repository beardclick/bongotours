...
export function BookingWidget({tourSlug,tourName,personPrice,groupPrice,priceType,seasonPrices,minPeople=1,image,priceOnRequest}:{tourSlug:string;tourName:string;personPrice:number;groupPrice?:number;priceType:'person'|'group';seasonPrices?:string;minPeople?:number;image?:string;priceOnRequest?:boolean}) {
  ...
  const whatsapp = `https://wa.me/50760909741?text=${encodeURIComponent(`Hola Bongo Outdoors, necesito más información sobre ${tourName}.`)}`;
  ...
  return <aside className="booking-card" id="reservar">{priceOnRequest?<><p className="eyebrow"><span /> Reserva segura</p><h2>Precio a consultar</h2><p className="booking-mode booking-mode--consult">Indícanos la fecha y el número de personas por WhatsApp y te confirmamos la tarifa disponible.</p><a className="button button--whatsapp full" href={whatsapp} target="_blank" rel="noreferrer">Consultar por WhatsApp <b>↗</b></a><small>Sin pago en línea. Nuestro equipo confirmará disponibilidad y precio antes de reservar.</small></>:<><p className="eyebrow"><span /> Reserva segura</p><h2>Book now</h2><div className="price-line"><strong>${unitPrice.toFixed(2)}</strong><span> / {mode === 'group' ? 'grupo' : 'persona'}</span></div><div className={`booking-mode booking-mode--${mode}`}>Tarifa fija por {mode === 'group' ? 'grupo' : 'persona'}</div><form onSubmit={addToCart}><label>Fecha<DatePicker value={date} onChange={setDate}/></label>{season && <p className="season-note">✦ Aplica {season.name}</p>}<label>Cantidad de personas<div className="stepper"><button type="button" aria-label="Restar persona" onClick={() => setQty(Math.max(1,qty-1))}>−</button><b>{qty}</b><button type="button" aria-label="Agregar persona" onClick={() => setQty(qty+1)}>+</button></div></label><div className="booking-total"><span>Total estimado</span><strong>${total.toFixed(2)}</strong></div><button className="button button--primary full" type="submit">Book now · Añadir al carrito <b>→</b></button><a className="button button--whatsapp full" href={whatsapp} target="_blank" rel="noreferrer">Más información por WhatsApp <b>↗</b></a><small>No se cobra en línea. Confirmarás el pedido desde el carrito y el pago será en efectivo.</small>{status && <p className="form-status" role="status">{status}</p>}</form></>}</aside>;
}
