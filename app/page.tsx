...
export default async function Home() {
  const [tours,hero]=await Promise.all([getTours(true),getPageConfig('home')]);
  ...
  return (
    <main>
      ...
      <section className="hero" aria-labelledby="hero-title">
        {hero.heroConfig?.type==='video'&&hero.heroConfig.videoUrl?<video className="hero__media" autoPlay muted loop playsInline src={hero.heroConfig.videoUrl}/>:hero.heroConfig?.type==='slideshow'&&(hero.heroConfig.slides?.length??0)>0?<HeroSlideshow slides={hero.heroConfig.slides!}/>:<div className="hero__bg" style={{backgroundImage:`url(${hero.image})`}}/>}
      ...
      <section className="story"><div className="story__image"><img src="https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=1400&q=85" alt="Guía de montaña observando el paisaje"/></div><div className="story__copy"><p className="eyebrow"><span/> {hero.sections.story.eyebrow}</p><h2>{hero.sections.story.title}</h2><p>{hero.sections.story.text}</p><Link className="button" href="/responsabilidad-social">Nuestra historia <b>→</b></Link><div className="stats"><div><strong>13+</strong><span>Años explorando</span></div><div><strong>4.9</strong><span>Valoración promedio</span></div><div><strong>8k+</strong><span>Viajeros felices</span></div></div></div></section>
      ...
      <section className="home-faq"><div className="shell home-faq__grid"><div><p className="eyebrow eyebrow--light"><span/> {hero.sections.faq.eyebrow}</p><h2>{hero.sections.faq.title}</h2><p>{hero.sections.faq.text}</p><Link className="button button--primary" href="/faqs">Ver todas las FAQs</Link></div><FaqList compact/></div></section>
      <Footer/><WhatsApp/>
    </main>
  );
}
