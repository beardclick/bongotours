...
export default async function ServicePage({ params }: { params: Promise<{slug: string}> }) {
  ...
  return <main>
    <Header transparent />
    <section className="tour-hero service-hero" style={{ backgroundImage: `url(${service.image})` }}><div className="tour-hero__shade" /><div className="shell"><p className="eyebrow eyebrow--light"><span /> Servicio Bongo Outdoors</p><h1>{service.h1}</h1><div className="tour-hero__actions"><a href="#cotizar" className="button button--primary">Cotizar ahora <b>↓</b></a><ShareButtons title={service.name} /></div></div></section>
    <section className="fact-strip"><div className="shell"><div><span>⌖</span><small>Cobertura</small><b>{service.location}</b></div><div><span>◷</span><small>Duración</small><b>{service.duration}</b></div><div><span>◇</span><small>Modalidad</small><b>{service.priceType === 'person' ? 'Por persona' : service.priceType === 'group' ? 'Por grupo' : 'Por servicio'}</b></div><div><span>✦</span><small>Atención</small><b>Personalizada</b></div></div></section>
    <section className="tour-detail shell"><article className="tour-copy"><p className="eyebrow"><span /> El servicio</p><h2>Haz que cada detalle de tu experiencia cuente.</h2><p className="lead">{service.description}</p><p>Nuestro equipo coordina contigo el alcance, la ubicación, el horario y los entregables. Antes de confirmar revisamos disponibilidad, condiciones del lugar y cualquier requisito especial.</p><div className="detail-columns"><div><h3>✓ Qué incluye</h3><ul>{service.includes.map(item => <li key={item}>{item}</li>)}</ul></div><div><h3>◎ Ideal para</h3><ul>{service.idealFor.map(item => <li key={item}>{item}</li>)}</ul></div></div><div className="detail-block"><h3>Disponibilidad y condiciones</h3><p>La operación depende del clima, permisos aplicables y condiciones de seguridad. Te confirmaremos todos los detalles antes de aceptar el servicio.</p></div></article>
