...
import { getLodgings as getRows } from './content';
...
export async function getStays(){const rows=await getRows() as StayRow[];const custom=rows.map(rowToStay);return[...custom,...defaultStays.filter(d=>!custom.some(c=>c.slug===d.slug))];}
