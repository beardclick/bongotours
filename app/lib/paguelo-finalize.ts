...
export async function finalizePagueloOrder(checkoutKey:string,paymentReference:string,confirmed=false){
  const db=await ensureDatabase();await ensureCouponSchema(db);
  const pendingRow=await db.prepare('SELECT payload FROM pending_payments WHERE provider=? AND reference=?').bind('paguelo',checkoutKey).first<{payload:string}>();
  ...
  const key=String(payload.checkoutKey||checkoutKey);
  ...
  const previous=await db.prepare('SELECT * FROM bookings WHERE checkout_key=? ORDER BY id').bind(key).all();
  if(previous.results.length){
    if(confirmed){
      const now=new Date().toISOString();
      const statements=[];
      for(const row of previous.results as BookingRow[]){
        if(row.status==='confirmed')continue;
        statements.push(db.prepare(`UPDATE bookings SET status='completed',updated_at=? WHERE reference=?`).bind(now,row.reference));
        statements.push(db.prepare(`INSERT INTO booking_status_history (booking_reference,status,note,changed_by,created_at) VALUES (?,'completed','Pago aprobado por Paguelo Fácil (webhook)',?,?)`).bind(row.reference,String(contact.email),now));
      }
      if(statements.length)await db.batch(statements);
    }
    await db.prepare(`UPDATE pending_payments SET status='completed' WHERE provider='paguelo' AND reference=?`).bind(checkoutKey).run();
    const updated=await db.prepare('SELECT * FROM bookings WHERE checkout_key=? ORDER BY id').bind(key).all();
    return{ok:true as const,orders:updated.results as BookingRow[],recovered:true,contact,items,total,subtotal:payload.subtotal,discount:payload.discount,couponCode:payload.couponCode};
  }
  ...
  const couponResult=couponCode?await validateCoupon(db,{code:couponCode,items:cartItems,email,userId:user?.userId,checkoutKey:key}):null;
  ...
  const note=confirmed?'Pago aprobado por Paguelo Fácil (webhook)':'Pago con tarjeta (Paguelo) — pendiente de verificación';
  const statements=[];
  for(const r of records){statements.push(db.prepare(`INSERT INTO bookings (reference,user_id,tour_slug,customer_name,email,phone,tour_date,quantity,price_mode,total,payment_method,payment_reference,coupon_code,discount_amount,cart_subtotal,checkout_key,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(r.reference,user?.userId??null,r.tourSlug,String(contact.customerName),email,String(contact.phone),r.tourDate,r.quantity,r.priceMode,r.total,'paguelo',paymentReference,couponCode||null,r.discount,cartSubtotal,key,status,now,now));statements.push(db.prepare(`INSERT INTO booking_status_history (booking_reference,status,note,changed_by,created_at) VALUES (?,?,?,?,?)`).bind(r.reference,status,note,email,now));}
  if(coupon&&records[0])statements.push(db.prepare(`INSERT OR IGNORE INTO coupon_redemptions (coupon_id,checkout_key,booking_reference,user_id,email,discount_amount,created_at) VALUES (?,?,?,?,?,?,?)`).bind(coupon.coupon.id,key,records[0].reference,user?.userId??null,email,paidDiscount,now));
  statements.push(db.prepare(`UPDATE pending_payments SET status='completed' WHERE provider='paguelo' AND reference=?`).bind(checkoutKey));
  await db.batch(statements);
  const saved=await db.prepare('SELECT * FROM bookings WHERE checkout_key=? AND email=? ORDER BY id').bind(key,email).all();
